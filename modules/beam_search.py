...
class BeamSearchNeuralWalker(object):
    ...
    def __init__(self, settings):
        ...
        if settings['path_model'] != None:
            for model_path in settings['path_model']:
                with open(model_path, 'rb') as f:
                    self.model_list.append(pickle.load(f))
        else:
            assert(settings['trained_model']!=None)
            self.model_list.append(settings['trained_model'])
        #
        ...
        for model in self.model_list:
            for param_name in model:
                model[param_name] = numpy.float32(model[param_name])
            #
            self.dim_model = model['Emb_enc_forward'].shape[1]
            #
            self.ht_encode_list.append(numpy.zeros(
                (self.dim_model, ), dtype=dtype
            ))
            self.ct_encode_list.append(numpy.zeros(
                (self.dim_model, ), dtype=dtype
            ))
            #
        self.scope_att = None
    ...
    def refresh_state(self):
        ...
        for ht_encode in self.ht_encode_list:
            ht_encode = numpy.zeros(
                (self.dim_model, ), dtype=dtype
            )
        for ct_encode in self.ct_encode_list:
            ct_encode = numpy.zeros(
                (self.dim_model, ), dtype=dtype
            )
        #
        self.scope_att = None
    ...
    def set_encoder_forward(self):
        self.ht_enc_forward_list = []
        self.ct_enc_forward_list = []
        for model in self.model_list:
            xt_lang_forward = model['Emb_enc_forward'][
                self.seq_lang_numpy, :
            ]
            shape_encode = xt_lang_forward.shape
            self.ht_enc_forward_list.append(numpy.zeros(
                shape_encode, dtype=dtype
            ))
            self.ct_enc_forward_list.append(numpy.zeros(
                shape_encode, dtype=dtype
            ))
            len_lang = shape_encode[0]
            for time_stamp in range(-1, len_lang-1, 1):
                post_transform = model['b_enc_forward'] + numpy.dot(
                    numpy.concatenate(
                        (
                            xt_lang_forward[time_stamp+1, :],
                            self.ht_enc_forward_list[-1][time_stamp, :]
                        ),
                        axis=0
                    ),
                    model['W_enc_forward']
                ...
                self.ct_enc_forward_list[-1][time_stamp+1, :] = gate_forget_numpy * self.ct_enc_forward_list[-1][time_stamp, :] + gate_input_numpy * gate_pre_c_numpy
                self.ht_enc_forward_list[-1][time_stamp+1, :] = gate_output_numpy * numpy.tanh(self.ct_enc_forward_list[-1][time_stamp+1, :])
            #
        #
    #
    ##
    def set_encoder_backward(self):
        self.ht_enc_backward_list = []
        self.ct_enc_backward_list = []
        for model in self.model_list:
            xt_lang_backward = model['Emb_enc_backward'][
                self.seq_lang_numpy, :
            ][::-1, :]
            shape_encode = xt_lang_backward.shape
            self.ht_enc_backward_list.append(numpy.zeros(
                shape_encode, dtype=dtype
            ))
            self.ct_enc_backward_list.append(numpy.zeros(
                shape_encode, dtype=dtype
            ))
            len_lang = shape_encode[0]
            for time_stamp in range(-1, len_lang-1, 1):
                post_transform = model['b_enc_backward'] + numpy.dot(
                    numpy.concatenate(
                        (
                            xt_lang_backward[time_stamp+1, :],
                            self.ht_enc_backward_list[-1][time_stamp, :]
                        ),
                        axis=0
                    ),
                    model['W_enc_backward']
                )
                #
                gate_input_numpy = self.sigmoid(
                    post_transform[:self.dim_model]
                ...
                self.ct_enc_backward_list[-1][time_stamp+1, :] = gate_forget_numpy * self.ct_enc_backward_list[-1][time_stamp, :] + gate_input_numpy * gate_pre_c_numpy
                self.ht_enc_backward_list[-1][time_stamp+1, :] = gate_output_numpy * numpy.tanh(self.ct_enc_backward_list[-1][time_stamp+1, :])
                #
            #
    #
    #
    ...
    def set_encoder(
        ...
        self.scope_att_list = []
        self.scope_att_times_W_list = []
        for idx, model in enumerate(self.model_list):
            self.scope_att_list.append(numpy.concatenate(
                (
                    self.Emb_lang_sparse[self.seq_lang_numpy, :],
                    self.ht_enc_forward_list[idx],
                    self.ht_enc_backward_list[idx][::-1, :]
                ),
                axis=1
            ))
            self.scope_att_times_W_list.append(numpy.dot(
                self.scope_att_list[-1], model['W_att_scope']
            ))
    ...
    def init_beam(self, pos_start, pos_end):
        ...
        item['htm1'] = []
        item['ctm1'] = []
        for idx, _ in enumerate(self.model_list):
            item['htm1'].append(self.ht_encode_list[idx])
            item['ctm1'].append(self.ct_encode_list[idx])
        ...
        for idx, model in enumerate(self.model_list):
            ...
            weight_current_step = self.softmax(
                numpy.dot(
                    numpy.tanh(
                        numpy.dot(
                            htm1_action[idx], model['W_att_target']
                        ) + self.scope_att_times_W_list[idx]
                    ),
                    model['b_att']
                )
            )
            ...
            zt_action = numpy.dot(
                weight_current_step,
                self.scope_att_list[idx]
            )
            #
            post_transform = model['b_dec'] + numpy.dot(
                numpy.concatenate(
                    (
                        xt_action, htm1_action[idx], zt_action
                    ),
                    axis=0
                ),
                model['W_dec']
            ...
            ct_action.append(gate_forget_numpy * ctm1_action[idx] + gate_input_numpy * gate_pre_c_numpy)
            ht_action.append(gate_output_numpy * numpy.tanh(ct_action[-1]))
            #
            post_transform_prob = numpy.dot(
                xt_action + numpy.dot(
                    numpy.concatenate(
                        (ht_action[-1], zt_action), axis=0
                    ),
                    model['W_out_hz']
                ),
                model['W_out']
            ...
            probt_list.append(exp_post_trans / numpy.sum(exp_post_trans))
            log_probt_list.append(numpy.log(probt_list[-1] + numpy.float32(1e-8)))

        probt = probt_list[0]
        for prob_next in probt_list[1:]:
            probt = probt + prob_next
        probt = probt / len(probt_list)

        log_probt = log_probt_list[0]
        for log_prob_next in log_probt_list[1:]:
            log_probt = log_probt + log_prob_next
        log_probt = log_probt / len(log_probt_list)
    ...
    def get_best_item(self):
        return self.finish_list[0]

    def check_pos_end_multiple_model(self, finish_list_best):
        top_path = finish_list_best
        diff_pos = numpy.sum(
            numpy.abs(
                top_path['pos_current'] - top_path['pos_destination']
            )
